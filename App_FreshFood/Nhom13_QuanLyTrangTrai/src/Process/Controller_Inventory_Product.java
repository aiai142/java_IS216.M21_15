...
public class Controller_Inventory_Product {
    ...
    public static List<String> name_product() throws SQLException, ClassNotFoundException{
        List<String> nameList = new ArrayList<>();
        ...
        CallableStatement stmt = conn.prepareCall("{call find_all_name_pro(?)}");
        stmt.registerOutParameter(1, oracle.jdbc.OracleTypes.CURSOR);
        stmt.execute();
        ResultSet rs = (ResultSet) stmt.getObject(1);
        while (rs.next()) {
            nameList.add(rs.getString("proName"));
        }
    ...
    public static String get_name_product(String id) throws SQLException, ClassNotFoundException{
      ...
      //Lay ten nguyen vat lieu dua theo ma nguyen vat lieu
    public static String get_id_product(String name) throws SQLException, ClassNotFoundException{
        Connection conn = null;
        PreparedStatement statement = null;
        conn = (Connection) ConnectionUtils.getMyConnection();
        
        CallableStatement stmt = conn.prepareCall("{call ?:= id_product(?)}");
    ...
    public static ArrayList<Inventory_Product> find_inven_pro(Inventory_Product pro) throws ClassNotFoundException, SQLException {
        ...
        stmt.setString(2, pro.getStockId());
        stmt.setString(3, pro.getProId());
       
        
        if(pro.getNum_inventory_pro() == null){
        stmt.setString(4, null);}else
        {
             stmt.setInt(4, pro.getNum_inventory_pro());
        }
