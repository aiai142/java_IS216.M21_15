...
public class Controller_Inventory_Resources {
    ...
    public static List<String> id_resources() throws SQLException, ClassNotFoundException{
     ...
     public static List<String> name_resources() throws SQLException, ClassNotFoundException{
       ...
       CallableStatement stmt = conn.prepareCall("{call find_all_name_re(?)}");
        stmt.registerOutParameter(1, oracle.jdbc.OracleTypes.CURSOR);
        stmt.execute();
        ResultSet rs = (ResultSet) stmt.getObject(1);
       while (rs.next()) {
            nameList.add(rs.getString("resourcesName"));
        }
    ...
    public static String get_id_resources(String name) throws SQLException, ClassNotFoundException{
        Connection conn = null;
        PreparedStatement statement = null;
        conn = (Connection) ConnectionUtils.getMyConnection();
        
        CallableStatement stmt = conn.prepareCall("{call ?:= id_Resources(?)}");
   ...
   public static ArrayList<Inventory_Resources> find_inven_Rc(Inventory_Resources rc) throws ClassNotFoundException, SQLException {
       ...
       stmt.setString(1, rc.getName());
       stmt.setString(2, rc.getStockId());
       stmt.setString(3, rc.getReId());
       if (rc.getNum_inventory_re() == null) {
           stmt.setString(4, null);
       } else {
           stmt.setInt(4, rc.getNum_inventory_re());
       }
