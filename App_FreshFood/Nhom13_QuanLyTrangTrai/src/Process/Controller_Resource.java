...
//        boolean check = true;
    ...
    // Ham tim nguyen vat lieu
    public static ArrayList<Resources> searchResources(String maVL, String tenVL, String loaiVL, String giaVL) 
                                                                                                                        throws SQLException, ClassNotFoundException {
        ArrayList<Resources> ResourcesList = new ArrayList<>();
        Connection conn = ConnectionUtils.getMyConnection();
        PreparedStatement statement = null;

        CallableStatement stmt = conn.prepareCall("{call search_Resources_forAll(?, ?, ?, ?, ?)}");
        stmt.setString(1, maVL);
        stmt.setString(2, tenVL);
        stmt.setString(3, loaiVL);
        stmt.setString(4, giaVL);
