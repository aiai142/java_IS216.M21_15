 ...
 * @author DoQuynhChi
 */
public class NV_ChooseProductPage extends javax.swing.JFrame {
     ...
     */
    public NV_ChooseProductPage(String id) throws SQLException, ClassNotFoundException {
        initComponents();
    ...
    */
                  ...
                  for (int col = 0; col < j; col++) {
                      if (panelHolder[row][col].jButtonGiam == evt.getSource()) {
                          ...
                          if (clicked[row][col] == 0) {
                              tableModel.removeRow(rowList.indexOf(NV_ChooseProductPage.JNode.this));
                              rowList.remove(NV_ChooseProductPage.JNode.this);

                              // khong cho nguoi dung nhan nut - khi so luong chon mua cua 1 JNode giam ve 0
                              jButtonGiam.setEnabled(false);

                          } else {
                              tableModel.setValueAt(clicked[row][col], rowList.indexOf(NV_ChooseProductPage.JNode.this), 1);
                              tableModel.setValueAt(arrLProductDB.get((row * j) + col).getProPrice() * clicked[row][col], rowList.indexOf(NV_ChooseProductPage.JNode.this), 2);
    ...
    private void jButtonXacNhanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonXacNhanActionPerformed
        ...
        if (total != 0) {
            ...
            try {
                new NV_NewOrderExport(total, id_login).setVisible(true);
            } catch (SQLException ex) {
                Logger.getLogger(NV_ChooseProductPage.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        this.setVisible(false);
        try {
            new AdminHome_1(id_login).setVisible(true);
        } catch (SQLException ex) {
            Logger.getLogger(NV_ChooseProductPage.class.getName()).log(Level.SEVERE, null, ex);
        } catch (ClassNotFoundException ex) {
            Logger.getLogger(NV_ChooseProductPage.class.getName()).log(Level.SEVERE, null, ex);
