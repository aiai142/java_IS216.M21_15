 ...
 * @author DoQuynhChi
 */
public class KH_ChooseProductPage extends javax.swing.JFrame {
     ...
     */
    public KH_ChooseProductPage(String id) throws SQLException, ClassNotFoundException {
        initComponents();
                  ...
                  for (int col = 0; col < j; col++) {
                      if (panelHolder[row][col].jButtonGiam == evt.getSource()) {
                          ...
                          if (clicked[row][col] == 0) {
                              tableModel.removeRow(rowList.indexOf(KH_ChooseProductPage.JNode.this));
                              rowList.remove(KH_ChooseProductPage.JNode.this);

                              // khong cho nguoi dung nhan nut - khi so luong chon mua cua 1 JNode giam ve 0
                              jButtonGiam.setEnabled(false);

                          } else {
                              tableModel.setValueAt(clicked[row][col], rowList.indexOf(KH_ChooseProductPage.JNode.this), 1);
                              tableModel.setValueAt(arrLProductDB.get((row * j) + col).getProPrice() * clicked[row][col], rowList.indexOf(KH_ChooseProductPage.JNode.this), 2);
    ...
    private void jButtonXacNhanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonXacNhanActionPerformed
        ...
        if (total != 0) {
            try {
                ...
                new KH_NewOrderExport(total, id_login).setVisible(true);
            } catch (SQLException ex) {
                Logger.getLogger(KH_ChooseProductPage.class.getName()).log(Level.SEVERE, null, ex);
