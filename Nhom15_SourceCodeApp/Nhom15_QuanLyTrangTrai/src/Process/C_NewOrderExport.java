...
public class C_NewOrderExport {
    ...
    public static ArrayList<Product> arrLProPurchase = new ArrayList<>();
    ...
    */
     ...
     */
     ...
     public static String themOrderExport(Order_Export ord) throws SQLException, ClassNotFoundException {
        ...
        stmt.setString(8, ord.getDeli_Address());
        stmt.setString(9, ord.getPayBy());

        stmt.execute();
        String ord_id = stmt.getString(1);
        
        conn.close();
        return ord_id;
    }
     
     public static String getUserName (String id_login) throws SQLException, ClassNotFoundException {
     ...
     public static long updateTotalAmount (DefaultTableModel tbCTHD) {
         long total = 0;
         for (int i = 0; i < tbCTHD.getRowCount(); i++) {
             long Amount = Long.parseLong(tbCTHD.getValueAt(i, 2) + "");
             total = Amount + total;
         }
    ...
    * Ham luu thong tin cac san pham va so luong mua hang de them vao bang CTHD trong DB
     */
    public static void luuThongTinCTHD(DefaultTableModel tbCTHD) {
        for (int i = 0; i < tbCTHD.getRowCount(); i++) {
            String tenSP = (String) tbCTHD.getValueAt(i, 0);
            int soLuong = (int) tbCTHD.getValueAt(i, 1);
            int giaSP = (int) tbCTHD.getValueAt(i, 2);
